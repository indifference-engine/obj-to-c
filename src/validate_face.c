...
void validate_face(void) {
  ...
  const size_t *const face_vertices =
      index_v + number_of_indices - face_vertex_count;
  ...
  for (size_t first_index = 0; first_index < face_vertex_count; first_index++) {
    ...
    for (size_t second_index = first_index + 1;
         second_index < face_vertex_count; second_index++) {
      const size_t second_vertex = face_vertices[second_index];

      if (first_x == vertex_x[second_vertex] &&
          first_y == vertex_y[second_vertex] &&
          first_z == vertex_z[second_vertex]) {
  ...
  float first_xyz[3] = {vertex_x[first_vertex], vertex_y[first_vertex],
                        vertex_z[first_vertex]};

  const size_t second_vertex = face_vertices[1];
  float second_xyz[3] = {vertex_x[second_vertex], vertex_y[second_vertex],
                         vertex_z[second_vertex]};

  const size_t third_vertex = face_vertices[2];
  float third_xyz[3] = {vertex_x[third_vertex], vertex_y[third_vertex],
                        vertex_z[third_vertex]};
  ...
  for (size_t first_index = 0; first_index < face_vertex_count; first_index++) {
    ...
    float first_xyz[3] = {vertex_x[first_vertex], vertex_y[first_vertex],
                          vertex_z[first_vertex]};
    const size_t second_vertex =
        face_vertices[(first_index + 1) % face_vertex_count];
    float second_xyz[3] = {vertex_x[second_vertex], vertex_y[second_vertex],
                           vertex_z[second_vertex]};
    ...
    for (size_t second_index = first_index == face_vertex_count - 1 ? 1 : 0;
