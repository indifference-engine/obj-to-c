...
void register_material(void)
{
  ...
  else if (material_name_length > 7 && strncmp("cutout_", material_name, 7) == 0)
  {
    material_type = MATERIAL_TYPE_CUTOUT;
  }
  else if (material_name_length > 9 && strncmp("additive_", material_name, 9) == 0)
  {
    material_type = MATERIAL_TYPE_ADDITIVE;
  }
  else if (material_name_length > 8 && strncmp("blended_", material_name, 8) == 0)
  {
  ...
  {
    material_types = realloc_or_throw(material_types, sizeof(int) * (number_of_materials + 1));
    material_types[number_of_materials] = material_type;
    material_names = realloc_or_throw(material_names, sizeof(const char *) * (number_of_materials + 1));
